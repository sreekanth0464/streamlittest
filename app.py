...
class Dashboard:
    ...
    def Customers(*args):
        ...
        response2 = s3_client.get_object(Bucket='braintaprawdata', Key='customers_6months.csv')
        ...
        st.table(monthly_new_customers.drop(columns=['year_month']))

        # Convert 'created' to datetime if it's not already
        customers['created'] = pd.to_datetime(customers['created'])

        # Delinquent Customers Over Time
        customers['month'] = customers['created'].dt.to_period('M').astype(str)
        monthly_delinquency = customers.groupby('month')['delinquent'].sum().reset_index()
        monthly_delinquency.columns = ['Month', 'Delinquent Customers']

        st.subheader('Delinquent Customers Over Time')
        fig = px.line(
            monthly_delinquency,
            x='Month',
            y='Delinquent Customers',
            title="Delinquent Customers by Month",
            width=800,
            height=400,
        ...
        #     marker=dict(size=8, color='#EF553B'),
        #     text=monthly_delinquency['Delinquent Customers'],
        #     textposition='top center'
        # )
        fig.update_layout(
            xaxis_title='Month',
            yaxis_title='Delinquent Customers',
            barmode='group',
            bargap=0.15,
            bargroupgap=0.1
        )
    ...
    def subscriptions(*args):
        s3_client = boto3.client('s3')
        response1 = s3_client.get_object(Bucket='braintaprawdata', Key='subscriptions_6months.csv')
        response2= s3_client.get_object(Bucket='braintaprawdata', Key='customers_6months.csv')
    ...
    def payment(self):
        s3_client = boto3.client('s3')
        response = s3_client.get_object(Bucket='braintaprawdata', Key='both_success_fail.csv')
        ...
        with st.expander("VIEW DATA"):
            df2_filtered['month'] = pd.to_datetime(df2_filtered['month']).dt.date
        ...
        with total2:
            ...
            st.subheader("Count of Payment Statuses")
            if not status_counts.empty and 'succeeded' in status_counts and 'failed' in status_counts:
                ...
                sizes = [succeeded_count, failed_count]
    ...
    def financial(self):
        s3_client = boto3.client('s3')
        response = s3_client.get_object(Bucket='braintaprawdata', Key='financial.csv')
        content = response['Body'].read().decode('utf-8')
...
def main():
    s3_client = boto3.client('s3')
    response = s3_client.get_object(Bucket='braintaprawdata', Key='Untitled_report.csv')# Fetch the object from S3
